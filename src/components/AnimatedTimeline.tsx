...
export default function AnimatedTimeline({ data }: AnimatedTimelineProps) {
  ...
  const draw = useCallback(() => {
    ...
    if (selectedAsset) {
      ...
      if (asset) {
        ...
        if (trailData.length > 1) {
          ...
          // Year labels on trail
          if (width > 600) {
            g.selectAll('.trail-year')
              .data(trailData.slice(0, -1))
              .join('text')
    ...
    const bubbles = g.selectAll('.bubble')
      ...
      .attr('opacity', d => selectedAsset && selectedAsset !== d.name ? 0.2 : 0.75)
      ...
      .on('mouseleave', function (_, d) {
        d3.select(this)
          .attr('opacity', selectedAsset && selectedAsset !== d.name ? 0.2 : 0.75)
  ...
  }, [data, currentYear, selectedAsset, containerWidth, getRankedData]);
